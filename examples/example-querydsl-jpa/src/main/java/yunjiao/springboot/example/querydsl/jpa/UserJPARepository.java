package yunjiao.springboot.example.querydsl.jpa;
...
import yunjiao.springboot.extension.querydsl.jpa.JPAQueryRepositorySupport;
...
public class UserJPARepository extends JPAQueryRepositorySupport {
    ...
    public Optional<User> findSingleByName(String name) {
        JPAQuery<User> query = selectFrom(qUser);
        return getCurdExecutor().findOnlyOne(query, UserSpec.nameEq(name));
    }

    // 多条件,排序查询
    public List<User> findList(String name, Integer age, Date birthDate) {
        JPAQuery<User> query = selectFrom(qUser);
        QSpecification spec = UserSpec.nameLike(name).and(UserSpec.ageGoe(age), UserSpec.birthDate(birthDate));
    ...
    public Page<User> findQPage(Integer age) {
        ...
        return getCurdExecutor().findPage(countQuery, query, QPageRequest.of(0, 5, sort), UserSpec.ageGoe(age));
    }
    ...
    public Page<User> findPage(Integer age) {
        ...
        return findPage(query, PageRequest.of(0, 5, sort), UserSpec.ageGoe(age));
    }
    ...
    public long updateName(Long id, String newName) {
        ...
        return getCurdExecutor().update(update, UserSpec.idEq(id));
    }
    ...
    public void deleteUser(Integer age) {
        ...
        getCurdExecutor().applaySpecification(UserSpec.ageGoe(age), queryClause);
