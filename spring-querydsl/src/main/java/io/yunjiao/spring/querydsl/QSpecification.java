...
public interface QSpecification extends Serializable {
    ...
    /**
     * 第一个条件。相当于: {@code where 1=1}
     *
     * @return 新的实例
     */
    static QSpecification where() {
        return (builder) -> new BooleanBuilder();
    }

    /**
     * 第一个条件。如果参数是null, 相当于: {@code where arg}
     *
     * @param predicate 必须值
     * @return 新的实例
     */
    static QSpecification where(@NonNull Predicate predicate) {
     ...
     * @return 新的实例
     */
    static QSpecification where(@NonNull QSpecification spec) {
    ...
    default QSpecification andAnyOf(@Nullable Iterable<QSpecification> specifications) {
        QSpecification spec= StreamSupport.stream(specifications.spliterator(), false)
                .reduce(where(), QSpecification::or);
    ...
    default QSpecification orAllOf(@Nullable Iterable<QSpecification> specifications) {
        QSpecification spec= StreamSupport.stream(specifications.spliterator(), false)
                .reduce(where(), QSpecification::and);
